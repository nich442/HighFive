...
template <typename T, typename U = typename manipulator<T>::hdf5_type>
inline void
...
template <typename Container, typename T = typename manipulator<Container>::hdf5_type>
...
struct data_converter<
    ...
    typename std::enable_if<(
        std::is_same<T, typename manipulator<T>::hdf5_type>::value &&
...
struct data_converter<
    std::array<T, S>,
    typename std::enable_if<(
        std::is_same<T, typename manipulator<T>::hdf5_type>::value)>::type>
...
struct data_converter<boost::multi_array<T, Dims>, void>
    ...
    using value_type = typename manipulator<T>::hdf5_type;
...
struct data_converter<std::vector<T>,
    ...
    std::vector<typename manipulator<T>::hdf5_type> _vec_align;
};
...
struct data_converter<std::vector<Reference>, void> {
    ...
    std::vector<size_t> _dims;
    std::vector<typename manipulator<hobj_ref_t>::hdf5_type> _vec_align;
};
